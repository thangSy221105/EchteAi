...
def main():
    ...
    metrics = evaluate_model(model, loader, device, include_rpn=not args.skip_rpn_metrics)
    metrics["model_size_mb"] = model_state_size_mb(model)
    metrics["parameters"] = int(model.logical_parameter_count)
