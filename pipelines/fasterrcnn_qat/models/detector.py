...
from .backbones import build_fpn_backbone
...
def build_fasterrcnn_model(config):
    ...
    model = FasterRCNN(
        ...
        rpn_anchor_generator=_anchors(config),
...
def build_fasterrcnn_convnext(config):
    """Compatibility alias for older scripts that still call the old builder name."""
    return build_fasterrcnn_model(config)
